...
/* Found a NetPDL field usable on matching */    
    ...
    if (field->Mask != NULL)
    {
		...
		/*Modificacion UAH*/
		else if (header == OXM_OF_AMARU_LEVEL){
                uint8_t * m_value;
                int i=0;
                m_value = ((uint8_t*)pktin->data + field->Position);
                ofl_structs_match_amaru_level(pktout,header,m_value);
        }
        else if (header == OXM_OF_AMARU_AMAC){
             uint8_t * m_value;
             int i=0;
             m_value = ((uint8_t*)pktin->data + field->Position);
             ofl_structs_match_amaru_amac(pktout,header,m_value);
        }
...
extern "C" int nblink_packet_parse(struct ofpbuf * pktin,  struct ofl_match * pktout, struct protocols_std * pkt_proto)
{
    ...
    while (proto!= NULL)
    {
        /* Getting first field of the protocol  */
			...
			/*Modificacion UAH*/
			else if (protocol_Name.compare("amaru") == 0 && pkt_proto->amaru == NULL)
            {
                pkt_proto->amaru = (struct Amaru_header *) ( (uint8_t*) pktin->data + proto->Position);
				PDMLReader->GetPDMLField(proto->Name, (char*) "Level", proto->FirstField, &field);
                nblink_extract_proto_fields(pktin, field, pktout, OXM_OF_AMARU_LEVEL);
                PDMLReader->GetPDMLField(proto->Name, (char*) "AMAC", proto->FirstField, &field);
                nblink_extract_proto_fields(pktin, field, pktout, OXM_OF_AMARU_AMAC);
            } 
			/*FIN Modificacion UAH*/			
